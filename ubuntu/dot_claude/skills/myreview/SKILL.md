...
- 収集する事項:
  - **最終番号 N**: 過去コメントに含まれる指摘番号 (`#N` / `No.N`) の最大値。無ければ 0
...
- 新規指摘が 0 件でも投稿する (「新規指摘なし。前回 #N までの対応を確認」の形で)
